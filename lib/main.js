...
function getExtensionsForScope(scopeName) {
...
module.exports = {
  ...
  getFiles(file) {
    ...
    const extname = path.extname(editor.getURI())
    if (extname) extensions.push(extname.substr(1))

    extensions.push(...getExtensionsForScope(editor.getGrammar().scopeName))
    const files = extensions.map(ext => `${file}.${ext}`)
    return uniq([...files, file])
  },
  ...
  detectFilePath(filePath) {
    ...
    const file = this.getFiles(filePath).find(f => fs.isFileSync(f))
    if (file) return file
  ...
  getFilePath(editor, fileName) {
    ...
    const filePath = this.detectFilePath(path.resolve(dirName, fileName))
    ...
    if (["source.sass", "source.css.scss"].includes(scopeName)) {
      ...
      return this.detectFilePath(path.resolve(dirName, sassPartialFileName))
    }

    // git diff output
    if (fileName.match(/^[ab]\//)) {
      return this.detectFilePath(path.resolve(dirName, fileName.replace(/^[ab]\//, "")))
    }

    // Search from projectRoot
    for (const dir of atom.project.getPaths()) {
      if (dirName.startsWith(dir)) {
        return this.detectFilePath(path.resolve(dir, fileName))
      }
    }
    return null
  },
